...
- **铜锈时钟**: `右键点击` 可将未涂蜡的方块推进到下一个氧化阶段，`长按右键` 可通过界面面板将方块调整到任意氧化阶段。拥有 `256` 点耐久（每次变更状态消耗 1 点，创造模式不消耗），并可在生存模式中合成。
...
- **工具作用于整个流体储罐**: 若启用，对流体储罐方块使用蜜脾涂蜡，或用斧/砂纸除蜡、除锈（手持或机械手）时，会作用于整个多方块储罐，大型储罐不会被拆散。使用铜锈时钟调整氧化状态时也会作用于整个多方块储罐。
- **启用随机刻氧化**: 若关闭，机械动力的未氧化方块以及本模组添加的氧化变种方块将不再被随机刻氧化，氧化状态只能通过工具进行调整。
